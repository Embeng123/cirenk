...
const listProxy = [
    ...
    { path: '/id2', proxy: '172.232.238.169' },
    //tambahin sendiri
];
let proxyIP;
export default {
...
async function getAllConfigVless(hostName) {
    try {
        ...
        for (const entry of listProxy) {
            ...
            const vlessTls = `vless://PALESTINE\u0040${hostName}:443?encryption=none&security=tls&sni=${hostName}&fp=randomized&type=ws&host=${hostName}&path=${pathFixed}#${isp} (${country_code})`;
            const vlessNtls = `vless://PALESTINE\u0040${hostName}:80?path=${pathFixed}&security=none&encryption=none&host=${hostName}&fp=randomized&type=ws&sni=${hostName}#${isp} (${country_code})`;
